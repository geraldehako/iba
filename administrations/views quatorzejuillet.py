...
def etuclassetemplate(request, id, iid):
    ...
    # Récupérer l'année scolaire active
    statuta = Anneescolaires.objects.get(Statutannee='1')
    return render(request,'Pages/etudiants/listeclassetemplate.html', context={'etu':etud,'annee': statuta})
...
def typeinsteajout(request):
    if request.method == 'POST':
        ...
        fm = TypeinstRegistration()
    return render(request,'Pages/typeecoles/ajout.html',{'form':fm,'annee': statuta}  )
...
def typeecolemodif(request, id):
    ...
    return render(request, 'Pages/typeecoles/modif.html',{'form':fm,'annee': statuta})
